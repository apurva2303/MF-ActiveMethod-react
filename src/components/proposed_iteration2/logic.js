...
function check_date_for_investment(fund, date, month, SIP, cost, NAV, cum_units, invested_amt, dates_of_investment) {

    let units_bought = 0;

    if (Number(fund_investment_dates[fund]) <= date.getDate()) {

        // console.log("month", month, "date", date_to_ddmmyyyy(date), "fund investment on", fund_investment_dates[fund], at_least_one_investment_done)
